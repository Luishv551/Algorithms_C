...
void input() {
    if (_kbhit()) {
        int key = _getch();
        switch (key) {
            case 'w': case 72: direction = 'U'; break; // 'w' ou seta para cima
            case 's': case 80: direction = 'D'; break; // 's' ou seta para baixo
            case 'a': case 75: direction = 'L'; break; // 'a' ou seta para esquerda
            case 'd': case 77: direction = 'R'; break; // 'd' ou seta para direita
...
int main() {
    ...
    while (!gameOver) {
        ...
        input();
        logic();
        usleep(100000); // Controlar a velocidade do jogo (em microsegundos)
    }
